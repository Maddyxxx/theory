...
- при создании слайса, если явно не указывать массив, который он будет использовать, Go выделяет память для него в куче.
...
- небуферизованый канал будет блокировать пишущие в него поотоки пока ячейка для значения не опустошится для записи
